...
window.getEleDefaultOpt = function () {
    return {
        id: '',
        clazz: '',
        hash: '',
...
window.extendEleDefaultOpt = function (inputOpt) {
    var opt = getEleDefaultOpt();
    if (isNotNone(inputOpt)) {
        ...
        if (isNotNone(inputOpt.style)) {
            opt.style = $.extend({}, inputOpt.style);
        }
    }
    return opt;
};
...
window.Ele = function () {
    ...
    var opt = lastArgIsOpt ? extendEleDefaultOpt(lastArg) : getEleDefaultOpt();

    var tmplArgs = lastArgIsOpt ? args.slice(0, argsLen - 1) : args;

    var ele = tmplEle.apply(thiz, tmplArgs);
